<?php
    ...
    function getLists(){
        ...
        $allnames = array();
        $q = query("SELECT game, tables FROM seatingGames ORDER BY id ASC");
        foreach($q as $game){
        ...
        $q = query("SELECT game FROM seatingGames");
        foreach($q as $rows){
            echo "<option value=\"".
            $rows["game"]."\">".
            $rows["game"];
        ...
        <input type="text" id="markietext" value="<?php //echo $q[0]['id'] ?>"></input>
        ...
        // $unban = query("SELECT name, ip FROM `seatingBanned`");
        // foreach($unban as $un){
            // echo "<option value=\"".
            // $un["ip"]."\">".
            // $un["name"];
